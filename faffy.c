...
// updating the window title to the title outputed by the shell, given the shell returns one
#if VTE_CHECK_VERSION(0, 78, 0) // version dependent implementation to maintain backwards compatibility
#define WINDOW_TITLE_CHANGED "termprop-changed"
static void windowTitleChangedCallback(VteTerminal* terminal, const char* prop, gpointer user_data) {
	if(g_strcmp0(prop, VTE_TERMPROP_XTERM_TITLE) != 0) return;
...
int main(int argc, char* argv[]) {
	...
	size_t num_ansi = sizeof(ANSI_PALETTE) / sizeof(ANSI_PALETTE[0]);
	GdkRGBA palette[num_ansi];
	for(size_t i = 0; i < num_ansi; i++) gdk_rgba_parse(&palette[i], ANSI_PALETTE[i]);
